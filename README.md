...
- **`seguridad.sh`**: Endurecimiento con Firewalld (servicios `kdeconnect`, `mdns`, `ssh`, zona `libvirt` para virbr0, zona `trusted` para `podman+`), DNS-over-TLS y sysctl unprivileged ports.
